...
@router.get("/alumnos_F")
async def alumnos_F():
    return (users_list)


@router.post("/alumnos_F", response_model=User, status_code=status.HTTP_201_CREATED)
async def alumnos_A(user: User):
...
@router.put("/alumnos_F")
async def alumnos_F(user: User):
...
@router.delete("/alumnos_F/{id}")
async def alumnos_F(id: int):

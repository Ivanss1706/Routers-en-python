...
@router.get("/alumnos_B")
async def alumnos_B():
    return (users_list)


@router.post("/alumnos_B", response_model=User, status_code=status.HTTP_201_CREATED)
async def alumnos_B(user: User):
...
@router.delete("/alumnos_B/{id}")
async def alumnos_B(id: int):
